...
این پروژه شامل یک کلاستر ClickHouse با 3 نود و 3 نود ClickHouse Keeper برای مدیریت هماهنگ سازی است. همچنین ClickStack (Grafana با ClickHouse datasource) برای observability و monitoring اضافه شده است.
...
### فایل‌های ClickStack (Observability)
- **grafana/provisioning/datasources/clickhouse.yml**: تنظیمات خودکار datasource ClickHouse برای Grafana
...
### ClickStack (Grafana Observability)
- **Grafana Dashboard**: `3000` → رابط وب برای monitoring و observability
...
⚠️ **نکته**: در اولین ورود، Grafana از شما می‌خواهد پسورد admin را تغییر دهید.

برای اطلاعات بیشتر، به فایل `CLICKSTACK_SETUP.md` مراجعه کنید.
...
- **Authentication**: برای هر replica، `user=default` و `password=default` تعریف شده است
  - این برای استفاده از `cluster()` function ضروری است
  - بدون این تنظیمات، کوئری‌های `cluster()` خطای Authentication می‌دهند
...
- [Grafana ClickHouse Plugin](https://grafana.com/grafana/plugins/vertamedia-clickhouse-datasource/)

## 📖 مستندات اضافی

- **SETUP_GUIDE.md**: راهنمای گام‌به‌گام نصب و راه‌اندازی
- **CLICKSTACK_SETUP.md**: راهنمای استفاده از ClickStack (Grafana)
...
| `clickhouse-nodes.xml` | اضافه کردن `user` و `password` به هر replica | امکان استفاده از `cluster()` function - بدون این تنظیمات، کوئری‌های cluster() خطای Authentication می‌دهند |
| `docker-compose.yml` | اضافه کردن سرویس ClickStack (Grafana) | Observability و monitoring کلاستر |
| `grafana/provisioning/datasources/clickhouse.yml` | ایجاد فایل provisioning | اتصال خودکار Grafana به ClickHouse از طریق HAProxy |
